import { $, toast } from "./common.v1.js";
...
function loadPoster() {
  ...
  image.onerror = () => {
    ...
    $("sharePosterStatus").textContent = "海报载入失败，请关闭后重试。";
...
async function copyText() {
  ...
  if (copied) toast("推广文案已复制，带上这份可爱分享吧！");
  else {
    field.focus();
    field.select();
    $("shareError").textContent =
      "浏览器限制了自动复制，文案已选中。请使用 Ctrl+C（Mac 使用 ⌘C），手机可长按复制。";
    $("shareError").hidden = false;
  }
}
