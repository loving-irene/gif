...
// 分享到社区时本机动图的上传上限：与服务端的 shareImageLimit（4MB）保持一致。
// 超过这个体积的动图服务端也不收，所以前端不再“省这次上传”，避免两端上限不一致时
// 出现「本机以为不用传、服务端以为会传」的死角。
const shareUploadLimit = 4 * 1024 * 1024;

// 动作序列图规格（后台配置，经 /api/catalog 下发）：
// 4x4 共16格、每帧256×256；5x5 共25格、10x10 共100格，后两者每帧128×128。
// 未下发时回落 4×4。
const MOTION_SPECS = {
  "4x4": { cols: 4, frames: 16, size: 256 },
  "5x5": { cols: 5, frames: 25, size: 128 },
  "10x10": { cols: 10, frames: 100, size: 128 },
};
function motionSpec() {
...
async function encode(sheet) {
  ...
  try {
    ...
    return await new Promise((resolve, reject) => {
      const worker = new Worker("/assets/gif-worker.v5.js", { type: "module" });
      worker.onmessage = (e) => {
        worker.terminate();
