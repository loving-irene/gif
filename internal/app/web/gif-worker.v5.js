// GIF89a encoder v5. Stabilized subject motion, shared adaptive palette and spec timing.
// Literal LZW blocks reset before the code width grows, prioritizing correctness.
// 16/25帧保持80ms；100帧使用20ms，使完整动作仍约2秒且播放达到50 FPS。
const SPEC_BY_FRAMES = {
  16: { size: 256, delay: 8 },
  25: { size: 128, delay: 8 },
  100: { size: 128, delay: 2 },
};
...
export function encodeGIF(frames, width, height) {
  ...
  if (!spec || width !== spec.size || height !== spec.size)
    throw new Error(
      "动图必须为16帧每帧256×256，或25/100帧每帧128×128",
    );
