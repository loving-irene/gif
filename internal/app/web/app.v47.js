...
async function encode(sheet) {
  // 按后台配置的动作序列图规格切格。上游序列图固定为 1024×1024，
  // 无法整除时按比例取整划分格边界，与服务器合成一致。
  // 每格四周内缩约 5%，裁掉邻格渗边（如脚下露出下一格头顶）。
  ...
  try {
    ...
    for (let n = 0; n < count; n++) {
      ...
      const cw = sx1 - sx0,
        ch = sy1 - sy0;
      let ix = Math.max(1, Math.floor(cw / 20));
      let iy = Math.max(1, Math.floor(ch / 20));
      if (ix * 2 >= cw) ix = Math.floor(cw / 4);
      if (iy * 2 >= ch) iy = Math.floor(ch / 4);
      const sx = sx0 + ix,
        sy = sy0 + iy,
        sw = Math.max(1, cw - ix * 2),
        sh = Math.max(1, ch - iy * 2);
      const canvas = document.createElement("canvas");
      canvas.width = size;
      canvas.height = size;
...
async function renderGallery() {
  ...
  for (const work of works) {
    ...
    card.append(
      element("img", {
        src: url,
        alt: work.name + (work.gif ? " 动态作品" : " 动作序列原图"),
        loading: "lazy",
      }),
    );
    let sheetURL = work.sheet && !work.gif ? url : "";
    if (work.sheet && work.gif) {
      sheetURL = URL.createObjectURL(work.sheet);
      galleryURLs.push(sheetURL);
      const source = element("div", { className: "gallery-sheet" });
      source.append(
        element("p", {}, "动作序列原图"),
        element("img", {
          src: sheetURL,
          alt: work.name + " 动作序列原图",
          loading: "lazy",
        }),
      );
      card.append(source);
    ...
    if (work.sheet && sheetURL) {
      row.append(
        element(
          "a",
          {
            href: sheetURL,
            download: work.name + "-序列原图.png",
