...
function updateControls() {
  ...
  for (const n of document.querySelectorAll(
    ...
    if (n.id !== "draftBtn") n.disabled = running;
  }
  for (const n of document.querySelectorAll("#actions button"))
    n.disabled = running;
...
async function preparePhoto(file) {
  ...
  if (file.size > 50 * 1024 * 1024)
    throw new Error("原始照片过大，请选择小于50MB的照片");
  const bitmap = await createImageBitmap(file, {
    imageOrientation: "from-image",
  });
  try {
    if (bitmap.width * bitmap.height > 50000000)
      throw new Error("照片像素过大，请先缩小后再选择");
    ...
    $("photoInfo").textContent =
      `照片已在浏览器整理为 ${(blob.size / 1024 / 1024).toFixed(2)} MB，上传前移除照片元数据。脸型和五官保持不变。`;
    step(1);
    updateControls();
  } finally {
    bitmap.close();
...
async function poll(pending) {
  ...
  throw new Error("任务仍在处理中，请刷新页面恢复查询，不要重复创建。");
}
async function makeDraft() {
  ...
  progress(
    "正在制作专属于你的角色",
    "我们会保留你的面部特征，再为你换上喜欢的造型。通常需要几分钟，请保持页面打开。",
  );
  try {
    const input = {
  ...
  try {
    for (let i = 0; i < actions.length; i++) {
      ...
      progress(
        ...
        "每个动作单独生成；完成后自动保存到本机作品集。请保持此页面打开。",
      );
      const input = {
        requestId: crypto.randomUUID(),
        kind: "motion",
...
function events() {
  ...
  $("accountBtn").onclick = () => {
    $("accountInfo").textContent =
      `账号 ${account.id.slice(0, 8)} · 剩余 ${account.credits} 次${account.email ? " · " + account.email : " · 当前为设备账号"}`;
...
async function init() {
  try {
    ...
    if (!catalog.configured) {
      $("notice").textContent =
        "创作服务正在准备中。管理员配置图像服务后，就可以开始制作你的专属 GIF 了。";
      $("notice").hidden = false;
    }
    await restoreProfile();
    await renderGallery();
