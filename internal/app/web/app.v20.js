...
const tasks = new Map();
let taskClock = null;
// 动作批次状态：一批动作按账号并发额度并行提交，批次进行中不再接收新的批次。
...
function renderEstimates() {
  ...
  if (!count) {
    $("motionEstimate").textContent = "选择动作后显示整组预计用时。";
    return;
  }
...
function renderServiceStatus() {
  ...
  $("chargeHint").textContent =
    ...
    `当前账号最多可同时执行 ${catalog.userConcurrency || 5} 个任务，动作图会按并发额度并行生成；等待中也能继续修改造型并提交新的定稿。`;
}
...
function freeMotionSlots() {
  const limit = Number(catalog?.userConcurrency) || 5;
  return Math.max(0, limit - activeCreditTasks());
}
