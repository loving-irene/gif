...
function updateControls() {
  ...
  if ($("redraftBtn")) $("redraftBtn").disabled = !profile?.draft || atLimit;
  const count = selectedActions.size;
...
function showDraft() {
  ...
  $("draftInfo").hidden = false;
  $("draftImageWrap").hidden = false;
  updateControls();
}
let candidateURLs = [];
...
async function switchCandidate(c) {
  ...
  // 切换后留在定稿预览并刷新大图与选中态；不要跳去动作页，否则看起来像点了没反应。
  showDraft();
}
...
function events() {
  ...
  if ($("redraftBtn")) $("redraftBtn").onclick = makeDraft;
  ...
  $("backDraft").onclick = () => (profile?.draft ? showDraft() : showActions());
