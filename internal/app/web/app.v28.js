...
// 分享到社区时本机动图的上传上限（base64 膨胀后仍有富余）：超过就让服务端用作品副本，
// 避免手机上为一张大图传很久。
const shareUploadLimit = 3 * 1024 * 1024;
...
async function shareToCommunity(work, button) {
  ...
  // 服务端作品副本优先（省一次上传）；本机独有的作品把动图传上去。
  const payload = { id: work.id, action: work.name || "" };
  if (image.size <= shareUploadLimit && work.cloud !== true)
    payload.image = await blobData(image);
