...
function syncMotionPromptGrid() {
  ...
  const grid = $("motionGrid").value === "5x5" ? "5x5" : "4x4";
  const isFive = grid === "5x5";
  const clauses = [
...
const deploymentViews = {
  ...
  "newer-commit-available": {
    badge: "待部署",
    headline: "检测到尚未部署的新版本",
    tone: "warning",
  },
  stuck: {
    badge: "部署异常",
    headline: "本地代码已更新，但服务仍记录为旧版本",
    tone: "warning",
  },
  "stale-reference": {
    badge: "待确认",
    headline: "远端版本信息已过期，暂时无法确认",
    tone: "neutral",
  },
  unknown: {
    badge: "未知",
...
const deploymentReasons = {
  "invalid-branch": "跟踪分支配置无效。",
  "not-git-repository": "应用目录不是可读取的 Git 仓库。",
  "head-unavailable": "无法读取本地 HEAD 提交。",
  "no-origin-ref": "尚未找到远端分支记录。",
  "missing-state-file": "尚未找到部署成功状态文件。",
  "invalid-state-commit": "部署状态文件中的提交格式无效。",
  "unresolvable-state-commit": "部署状态文件中的提交不在当前仓库中。",
  "remote-reference-stale": "最近一次远端检查已超过 15 分钟，请检查自动部署任务。",
  "deployment-stuck": "上一次部署可能未成功，请检查自动部署日志。",
  "deployment-behind": "远端分支包含尚未上线的提交，等待自动部署完成。",
};
function deploymentCommitText(commit) {
  if (!commit) return "—";
  return `${commit.hash}${commit.subject ? ` · ${commit.subject}` : ""}${
    commit.committedAt ? ` · ${timeText(commit.committedAt)}` : ""
  }`;
}
async function loadDeploymentVersion() {
  const version = await api("/api/admin/deployment-version");
  const view = deploymentViews[version.status] || deploymentViews.unknown;
  $("deploymentBadge").className = "admin-badge " + view.tone;
  $("deploymentBadge").textContent = view.badge;
  $("deploymentHeadline").textContent = view.headline;
  $("deploymentDetail").textContent =
    deploymentReasons[version.reason] ||
    `当前部署提交与最近抓取的 origin/${version.branch} 一致。`;
  $("deploymentRows").replaceChildren();
  for (const [label, value] of [
    ["跟踪分支", `origin/${version.branch}`],
    ["当前部署", deploymentCommitText(version.deployed)],
    ["远端最新（最近抓取）", deploymentCommitText(version.remote)],
    ["本地 HEAD", deploymentCommitText(version.head)],
    ["远端检查时间", timeText(version.fetchedAt)],
    ["页面检查时间", timeText(version.checkedAt)],
  ]) {
    const row = element("tr");
    td(row, label);
    td(row, value);
    $("deploymentRows").append(row);
  }
}
