...
async function syncWorks() {
  ...
  const known = new Set(mine.filter((w) => w.gif?.size > 0 || w.sheet?.size > 0).map((w) => w.id));
  let added = 0;
  for (const work of items || []) {
    if (known.has(work.id)) continue;
    // 优先取 GIF；云端只有动作原图（GIF 合成失败）时取原图，本机可免费重新合成。
    let blob = null;
    let isGif = true;
    let r = await mediaResponse(
      `/api/works/${encodeURIComponent(work.id)}/gif`,
    ).catch(() => null);
    if (r?.ok) blob = await r.blob();
    else {
      isGif = false;
      r = await mediaResponse(
        `/api/works/${encodeURIComponent(work.id)}/sheet`,
      ).catch(() => null);
      if (r?.ok) blob = await r.blob();
    }
    if (!blob) continue;
