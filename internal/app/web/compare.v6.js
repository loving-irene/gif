...
function downloadButton(url, filename, label) {
  ...
  btn.type = "button";
  btn.className = "secondary cell-download";
...
function renderBatch(batch) {
  ...
  for (const job of jobs) {
    ...
    td.append(st);
    if (job.draftUrl) {
      td.append(downloadButton(job.draftUrl, `${safeFileStem(job.name)}_draft.png`, "下载定稿"));
    }
  ...
  for (const job of jobs) {
    ...
    td.append(st);
    if (job.animUrl) {
      td.append(downloadButton(job.animUrl, `${safeFileStem(job.name)}_anim.gif`, "下载动画"));
    }
    animRow.append(td);
  }
}
