...
// 分享到社区时本机动图的上传上限：与服务端的 shareImageLimit（8MB）保持一致。
// 超过这个体积的动图服务端也不收，所以前端不再“省这次上传”，避免两端上限不一致时
// 出现「本机以为不用传、服务端以为会传」的死角。
const shareUploadLimit = 8 * 1024 * 1024;

// 动作序列图规格（后台配置，经 /api/catalog 下发）：
// 4x4 共16格、每帧256×256；5x5 共25格、每帧128×128；10x10 共100格、每帧256×256。
// 未下发时回落 4×4。
const MOTION_SPECS = {
...
function motionSpec() {
  return MOTION_SPECS[catalog?.motionGrid] || MOTION_SPECS["5x5"];
}
...
function selection() {
  // 服装/分类/画风已从前台移除：提交空对象，由服务端补默认值。
  // 有定稿后沿用 profile.selection，保证与定稿凭证一致。
  if (profile?.selection && Object.keys(profile.selection).length)
...
// 是否还需要「新生成」定稿：已有定稿图+凭证且对应当前自拍时，选动作只复用，不重复生成。
// 未确认（accepted=false）只走 /api/accept，不重新画定稿。
function hasUsableDraft() {
  if (!profile?.draft || !profile?.receipt) return false;
  if (!selfie) return true;
  if (!profile.selfie) return true;
  return profile.selfie === selfie;
}
function needsDraft() {
  return !hasUsableDraft();
}
function pipelineCost(actionCount) {
  return actionCount + (needsDraft() ? 1 : 0);
}
function updateControls() {
  renderEstimates();
  const atLimit = taskLimitReached();
  if ($("draftBtn")) $("draftBtn").disabled = !validSelection() || !catalog?.configured || atLimit;
  const count = selectedActions.size;
  $("selectedCount").textContent = count;
  $("cost").textContent = pipelineCost(count);
  $("motionBtn").disabled =
    !count || !(selfie || profile?.selfie || profile?.draft) || !catalog?.configured || atLimit;
  $("motionBtn").textContent = needsDraft()
    ? "生成定稿并做成动画 ✦"
    : "做成动画 ✦";
  $("stopBtn").hidden = !motionSubmitting();
  const hint = $("chargeHint");
  if (hint) {
    hint.textContent = needsDraft()
      ? "首次会先生成定稿（1 次），再按所选动作各消耗 1 次。"
      : "已有定稿，本次只按所选动作各消耗 1 次。";
  }
  syncAccountActions();
  updateFeedback();
}
// 进行中的任务属于当前账号：退出登录会让它们失去归属（服务端仍会做完并保留 3 天，但换成
// 新设备账号后再也找不回来），所以在弹窗里单独禁用「退出当前账号」并说明原因。
// 账号入口本身、查看剩余次数与调用记录、保存账户名、关联邮箱登录都不受影响：
// 邮箱登录是账号合并，进行中的任务会跟着并入合并后的账号，不会失去归属。
function syncAccountActions() {
  const busy = tasks.size > 0;
  $("logoutBtn").disabled = busy;
  const hint = $("accountLockedHint");
  if (!hint) return;
  hint.hidden = !busy;
  hint.textContent = busy
    ? `有 ${tasks.size} 个任务正在制作中，为避免任务失去归属，任务完成前暂时不能退出登录。查看次数与调用记录不受影响。`
    : "";
}
...
function step(n) {
  ...
  $("setup").hidden = n !== 1;
  $("motionSection").hidden = n !== 2;
  // 有自拍或已有定稿即可进入动作选择。
  const ready = Boolean(selfie || profile?.draft);
...
function currentCategory() {
  return catalogCategories().find((c) => c.id === selectedCategory) || catalogCategories()[0];
}
// 展平全部分类动作，供名称查找与按已选 ID 提交（同 ID 保留首次出现）。
function catalogActionList() {
  if (catalog?.actions?.length) return catalog.actions;
  const out = [];
  const seen = new Set();
...
function actionsForCategory(id) {
  const c = catalogCategories().find((x) => x.id === id) || currentCategory();
  return c?.actions || [];
}
function renderActionCategories() {
  const node = $("actionCategories");
  if (!node) return;
  node.replaceChildren();
  const list = catalogCategories();
  if (!list.length) return;
  if (!list.some((c) => c.id === selectedCategory))
  ...
  for (const c of list) {
    ...
    b.onclick = () => chooseActionCategory(c.id);
    node.append(b);
  }
}
function chooseActionCategory(id) {
...
function renderCategoryActions() {
  ...
  for (const a of actionsForCategory(selectedCategory)) {
...
async function preparePhoto(file) {
  ...
  if (file.size <= 5 * 1024 * 1024) {
    ...
    // 换自拍后旧定稿不再直接用于新图：管线会重新生成定稿。
    if (profile && profile.selfie !== file) {
  ...
  try {
    ...
    if (profile && profile.selfie !== blob) {
      profile.accepted = false;
      profile.draft = null;
      profile.receipt = "";
    }
...
let candidateURLs = [];
// 已生成的定稿放在创作大框架内，与当前步骤无关；点选切换当前定稿，下方保存按钮下载对应图。
// IndexedDB 读写后 Blob 引用会变，不能靠 === 判断是否同一张定稿；优先用凭证。
function isCurrentCandidate(c) {
  if (!c || !profile) return false;
  if (c.receipt && profile.receipt) return c.receipt === profile.receipt;
  return c.draft === profile.draft;
}
function renderCandidates() {
  ...
  $("draftStripCount").textContent = list.length
    ? `已生成 ${list.length} 张 · 点击切换当前定稿`
    : "";
  const strip = $("draftCandidates");
  strip.replaceChildren();
  for (const c of list) {
    const card = element("div", { className: "candidate-card" });
...
async function switchCandidate(c) {
  if (isCurrentCandidate(c)) return;
  ...
  renderCandidates();
  // 留在当前步骤；若在动作页则同步分类，不再弹出单独大图。
  if (!$("motionSection").hidden) {
    const fromDraft = profile.selection?.category;
    const list = catalogCategories();
    if (fromDraft && list.some((cat) => cat.id === fromDraft)) {
      selectedCategory = fromDraft;
      renderActionCategories();
      renderCategoryActions();
    }
  }
...
function showActions() {
  const list = catalogCategories();
  if (!selectedCategory && list.length) selectedCategory = list[0].id;
  // 有定稿时优先回到定稿所属分类，方便继续挑同款世界的动作。
  const fromDraft = profile?.selection?.category;
  if (fromDraft && list.some((c) => c.id === fromDraft))
    selectedCategory = fromDraft;
  renderActionCategories();
  renderCategoryActions();
  step(2);
...
function actionLabel(id) {
  for (const a of catalogActionList()) if (a.id === id) return a.name;
  return id || "自定义动作";
}
...
const DRAFT_DETAIL =
  "我们会保留你的面部特征，生成专属定稿。通常需要几分钟。任务在服务器后台执行，你可以关闭页面稍后回来看。";
// 打开「增加次数」弹窗：二维码直接展示在弹窗顶部，扫码添加微信即可获得更多次数；
// 二维码懒加载——首次打开才请求，与页脚「联系我」共用同一张 /assets/wechat.v1.jpg。
// reason 不为空时在弹窗顶部展示醒目的原因提示（如“剩余次数不够”），说明为何弹到这里。
function openRedeem(reason = "") {
  const dialog = $("redeemDialog");
  $("redeemReason").textContent = reason;
  $("redeemReason").hidden = !reason;
  if (!dialog.open) dialog.showModal();
  const image = $("redeemQR");
  if (image && !image.getAttribute("src")) {
    $("redeemQRStatus").textContent = "正在载入二维码…";
    $("redeemQRStatus").hidden = false;
    image.src = "/assets/wechat.v1.jpg";
  }
}
async function makeDraft() {
  try {
    ...
    if (availableCredits() < 1) {
      openRedeem("剩余创作次数不够，无法生成新的角色定稿。扫下方二维码添加微信即可增加次数。");
      return;
    }
...
function isSubjectPixelRGBA(data, width, x, y) {
  const i = (y * width + x) * 4;
  if (data[i + 3] < 128) return false;
  return !(data[i] > 245 && data[i + 1] > 245 && data[i + 2] > 245);
}

function clearBottomBleedRGBA(data, width, height) {
  const rowCount = new Array(height).fill(0);
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++)
      if (isSubjectPixelRGBA(data, width, x, y)) rowCount[y]++;
  let cutFrom = -1;
  let bottom = -1;
  ...
  if (bottom >= 0) {
    let bleedTop = bottom;
    for (let y = bottom; y >= 0 && rowCount[y] > 0; y--) bleedTop = y;
    let gapEnd = bleedTop - 1;
    while (gapEnd >= 0 && rowCount[gapEnd] === 0) gapEnd--;
    const gapRows = bleedTop - 1 - gapEnd;
    const bleedH = bottom - bleedTop + 1;
    if (gapRows >= 2 && bleedH <= (height * 28) / 100 && gapEnd >= 0)
      cutFrom = bleedTop;
  }
  if (cutFrom < 0) {
    const lo = Math.floor((height * 70) / 100);
    let bestY = -1,
      bestCnt = width + 1;
    for (let y = lo; y < height - 1; y++)
    ...
    if (bestY > lo) {
      let peakAbove = 0,
        peakBelow = 0;
      for (let y = lo; y < bestY; y++) peakAbove = Math.max(peakAbove, rowCount[y]);
      for (let y = bestY + 1; y < height; y++)
        peakBelow = Math.max(peakBelow, rowCount[y]);
      const thresh = Math.max(3, (Math.max(peakAbove, peakBelow) * 15) / 100);
      if (
        peakAbove >= 12 &&
        peakBelow >= 12 &&
        bestCnt <= thresh &&
        height - 1 - bestY <= (height * 28) / 100
      )
        cutFrom = bestY + 1;
    }
  }
  if (cutFrom < 0) return;
  for (let y = cutFrom; y < height; y++)
    for (let x = 0; x < width; x++) data[(y * width + x) * 4 + 3] = 0;
}

function subjectBoundsRGBA(data, width, height) {
  let minX = width,
    minY = height,
    maxX = -1,
    maxY = -1;
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      if (!isSubjectPixelRGBA(data, width, x, y)) continue;
...
function fitCellToFrame(bitmap, sx0, sy0, cw, ch, size) {
  const src = document.createElement("canvas");
  src.width = cw;
  src.height = ch;
  const sctx = src.getContext("2d", { willReadFrequently: true });
  sctx.drawImage(bitmap, sx0, sy0, cw, ch, 0, 0, cw, ch);
  const img = sctx.getImageData(0, 0, cw, ch);
  clearBottomBleedRGBA(img.data, cw, ch);
  sctx.putImageData(img, 0, 0);
  let bbox = subjectBoundsRGBA(img.data, cw, ch);
  const out = document.createElement("canvas");
  out.width = size;
  out.height = size;
  const octx = out.getContext("2d", { willReadFrequently: true });
  if (!bbox) {
    const ix = Math.max(1, Math.floor(cw / 20));
    const iy = Math.max(1, Math.floor(ch / 20));
    octx.drawImage(
      bitmap,
      sx0 + ix,
      sy0 + iy,
    ...
    return octx.getImageData(0, 0, size, size).data.buffer;
  }
  const pad = Math.max(2, Math.floor(Math.min(cw, ch) / 16));
  bbox = {
    minX: Math.max(0, bbox.minX - pad),
    minY: Math.max(0, bbox.minY - pad),
    maxX: Math.min(cw, bbox.maxX + pad),
    maxY: Math.min(ch, bbox.maxY + pad),
  };
  ...
  const footBias = size * 0.02;
  let scale = Math.min(avail / bw, (avail + footBias) / bh);
  if (scale > 1.12) scale = 1.12;
  if (scale < 0.2) scale = 0.2;
  const tw = Math.max(1, Math.round(bw * scale));
  const th = Math.max(1, Math.round(bh * scale));
  const x0 = Math.floor((size - tw) / 2);
  let y0 = size - Math.round(margin) - th;
  if (y0 < Math.round(margin)) y0 = Math.round(margin);
  if (y0 + th > size) y0 = size - th;
  if (y0 < 0) y0 = 0;
  octx.drawImage(src, bbox.minX, bbox.minY, bw, bh, x0, y0, tw, th);
  return octx.getImageData(0, 0, size, size).data.buffer;
}

async function encode(sheet) {
  // 按后台配置的动作序列图规格切格。与服务器一致：清邻格渗边后按主体包围盒适配，脚略靠下。
  ...
  try {
    ...
    for (let n = 0; n < count; n++) {
      ...
      const sx0 = Math.floor((col * bitmap.width) / cols);
      const sy0 = Math.floor((row * bitmap.height) / cols);
      const sx1 = Math.floor(((col + 1) * bitmap.width) / cols);
      const sy1 = Math.floor(((row + 1) * bitmap.height) / cols);
      frames.push(
        fitCellToFrame(bitmap, sx0, sy0, sx1 - sx0, sy1 - sy0, size),
      );
    }
    return await new Promise((resolve, reject) => {
      const worker = new Worker("/assets/gif-worker.v7.js", { type: "module" });
      worker.onmessage = (e) => {
        worker.terminate();
...
async function ensureAcceptedDraft() {
  if (!selfie && profile?.selfie) selfie = profile.selfie;
  // 已有选定定稿：只确认凭证，绝不重新生成。
  if (hasUsableDraft()) {
    if (profile.accepted) return;
    const accepted = await api("/api/accept", { receipt: profile.receipt });
    profile.receipt = accepted.receipt;
    profile.accepted = true;
    for (const c of profile.candidates || []) {
      if (isCurrentCandidate(c)) {
        c.receipt = accepted.receipt;
        c.accepted = true;
      }
    }
    await local("profiles", "put", profile);
  ...
  if (!selfie) throw userNotice("请先上传自拍");
  const input = {
    requestId: crypto.randomUUID(),
    kind: "draft",
    selection: selection(),
  ...
  try {
    const result = await poll(task);
    await receiveDraft(result, task);
    if (!profile?.receipt) throw userNotice("定稿生成失败，请重试");
    const accepted = await api("/api/accept", { receipt: profile.receipt });
    profile.receipt = accepted.receipt;
    profile.accepted = true;
    for (const c of profile.candidates || []) {
...
async function makeMotions() {
  // 每次点击只提交本次选中的动作：无定稿时先自动定稿并确认，再生成动画。
  if (!selectedActions.size) return;
  if (!selfie && profile?.selfie) selfie = profile.selfie;
  const cost = pipelineCost(selectedActions.size);
  if (cost > availableCredits()) {
    openRedeem(
      `本次预计消耗 ${cost} 次（${needsDraft() ? "含定稿 1 次 + " : ""}${selectedActions.size} 个动作），超过剩余 ${Math.max(0, availableCredits())} 次。可减少动作，或扫下方二维码添加微信增加次数。`,
    );
    return;
  }
  if (taskLimitReached()) {
  ...
  const actions = catalogActionList().filter((a) => selectedActions.has(a.id));
  ...
  try {
    ...
    await ensureAcceptedDraft();
    if (actions.length > availableCredits()) {
      openRedeem(
        `已选 ${actions.length} 个动作，超过剩余 ${Math.max(0, availableCredits())} 次。可减少动作，或扫下方二维码添加微信增加次数。`,
      );
      return;
    }
...
// 已经分享到社区的作品编号 → 分享条目编号。仅用于作品集卡片的「分享 / 取消分享」状态，
// 取消分享只删社区池里的那条，不影响本机与云端作品。
const sharedWorks = new Map();
// 拉取当前账号已分享的作品；失败时按“都没分享”展示，点了分享会自然补上。
async function syncCommunityShares() {
  try {
    const { shares } = await api("/api/community/states");
    sharedWorks.clear();
    for (const [workID, shareID] of Object.entries(shares || {}))
      sharedWorks.set(workID, shareID);
  } catch {
    sharedWorks.clear();
  }
}
// 社区只收 GIF 动图：优先用本机/云端已有的 GIF，只有动作原图时先在浏览器合成为 GIF。
async function shareImageFor(work) {
  if (work.gif?.type === "image/gif") return work.gif;
  if (work.sheet) {
    work.gif = await encode(work.sheet);
    await local("works", "put", work);
  }
  return work.gif?.type === "image/gif" ? work.gif : null;
}
async function shareToCommunity(work, button) {
  const image = await shareImageFor(work);
  if (!image)
    throw userNotice("这张作品还没合成好动图，请先点「重新合成 · 免费」。");
  // 本机 GIF 一律随请求带上：服务端作品副本只保留 3 天，过期后客户端无法判断
  // 「服务端还有没有这张图」，只看 work.cloud 会让老作品必然分享失败。
  // 服务端仍优先用自己那份副本，所以刚生成的作品分享时不会真的多传一次。
  const payload = { id: work.id, action: work.name || "" };
  if (image.size <= shareUploadLimit) payload.image = await blobData(image);
  const item = await api("/api/community/share", payload);
  sharedWorks.set(work.id, item.id);
  if (button) {
    button.classList.add("share-state");
    toast("已分享到社区，其他人在社区页就能看到啦。");
  }
  await renderGallery();
}
async function unshareToCommunity(work) {
  await api("/api/community/unshare", { workId: work.id });
  sharedWorks.delete(work.id);
  toast("已取消分享，这张作品不再出现在社区。");
  await renderGallery();
}
...
async function renderGallery() {
  ...
  for (const work of works) {
    ...
    const main = work.gif || work.sheet;
    const url = URL.createObjectURL(main);
    galleryURLs.push(url);
    const meta = element("div", { className: "gallery-meta" });
    meta.append(
    ...
    // 分享到社区：已分享的显示「取消分享」，两种状态都只影响社区池，不动本机与云端作品。
    // 动作序列原图仅后台图库展示，前台作品集只展示 GIF。
    if (work.gif || work.sheet) {
      const shared = sharedWorks.has(work.id);
      const share = element(
        "button",
        {
          className: "text-button" + (shared ? " share-state" : ""),
          type: "button",
        },
        shared ? "取消" : "分享",
      );
      share.onclick = () =>
        busyButton(share, () =>
          shared
            ? unshareToCommunity(work)
            : shareToCommunity(work, share),
        );
      row.append(share);
    }
    ...
    remove.onclick = () => {
      if (
        !confirm(
          "移除会同时删除本机与云端的这张作品，其他设备同步后也不再显示。请先确保已下载保存。",
        )
      )
        return;
      busyButton(remove, async () => {
        // 云端副本一并删除；本机旧作品没有云端副本时忽略删除结果。
        await api("/api/works/remove", { id: work.id }).catch(() => {});
    ...
    card.append(
      ...
      element("img", {
        ...
        src: url,
        alt: work.name + (work.gif ? " 动态作品" : " 待合成原图"),
        loading: "lazy",
      }),
    );
    $("gallery").append(card);
...
async function restoreProfile() {
  ...
  renderCandidates();
  if (selfie || profile.draft) showActions();
  else step(1);
}
...
async function syncDrafts() {
  ...
  const known = new Set(
    (profile?.candidates || []).filter((c) => c.draft?.size > 0).map((c) => c.receipt).filter(Boolean),
  );
  const entries = [];
...
// 同一账号跨设备同步“我的作品集”：动作 GIF 生成成功时服务器会保存云端副本
//（最多保留 3 天，与任务结果文件一致），这里在窗口内拉取云端列表，
// 把本机没有的作品（按任务编号去重）合并进本机作品集。
// 保留期内从云端消失的作品视为被其他设备删除，本机同步副本一并移除；
// 超过保留期消失的是服务器到期清理，本机副本继续保留。
async function fetchWorkBlob(id, kind) {
  const r = await mediaResponse(
    `/api/works/${encodeURIComponent(id)}/${kind}`,
  ).catch(() => null);
...
async function syncWorks() {
  ...
  if (!res.ok) return 0;
  const { items, retentionSeconds } = await res.json();
  const retention = (Number(retentionSeconds) || 0) * 1000;
  const mine = (await local("works", "getAll")).filter(
    (w) => w.uid === account.id,
  );
  const byID = new Map(mine.map((w) => [w.id, w]));
  let added = 0;
  for (const work of items || []) {
    const localWork = byID.get(work.id);
    const hasLocal =
      localWork && (localWork.gif?.size > 0 || localWork.sheet?.size > 0);
    // 本机已有 GIF 但缺序列原图：云端仍有原图时补拉，便于图库展示/下载。
    if (hasLocal) {
      if (work.hasSheet && !(localWork.sheet?.size > 0)) {
        const sheet = await fetchWorkBlob(work.id, "sheet");
        if (sheet) {
          localWork.sheet = sheet;
          await local("works", "put", localWork);
          added++;
        }
      }
      continue;
    }
    // GIF 与动作序列原图都保留；列表字段缺失时仍尝试拉取（兼容旧接口）。
    const gif =
      work.hasGif === false ? null : await fetchWorkBlob(work.id, "gif");
    const sheet =
      work.hasSheet === false ? null : await fetchWorkBlob(work.id, "sheet");
    if (!gif && !sheet) continue;
    await local("works", "put", {
      id: work.id,
      uid: account.id,
      name: work.name,
      category: work.category,
      created: work.created * 1000,
      gif,
      sheet,
      // 标记为云端同步副本：保留期内从云端消失时随同步移除。
      cloud: true,
    });
    added++;
  }
  const live = new Set((items || []).map((w) => w.id));
  for (const w of mine)
    if (
      w.cloud &&
      retention > 0 &&
      !live.has(w.id) &&
      Date.now() - (w.created || 0) < retention
    )
      await local("works", "delete", w.id);
  return added;
}
...
function events() {
  ...
  if ($("draftBtn")) $("draftBtn").onclick = makeDraft;
  $("step1").onclick = () => step(1);
  $("step2").onclick = () => {
    if (selfie || profile?.draft) showActions();
    else step(2);
  };
  $("motionEmptyBtn").onclick = () => step(1);
  ...
  // 刷新作品：点击后图标旋转、按钮禁用；先同步云端作品集再重读本机数据，
  // 完成后按结果给出提示，让点击反馈和同步进度都可见。
  $("refreshGallery").onclick = () =>
    busyButton($("refreshGallery"), async () => {
      $("refreshGallery").classList.add("refreshing");
      try {
        let added = 0;
        try {
          added = await syncWorks();
        } catch {
          // 云端同步失败时仍然刷新本机数据。
        }
        await renderGallery();
        toast(
          added > 0
            ? `已从云端同步 ${added} 张作品，当前 ${$("galleryCount").textContent} 张`
            : `已刷新作品集，当前 ${$("galleryCount").textContent} 张`,
        );
      } finally {
        $("refreshGallery").classList.remove("refreshing");
      }
    });
  ...
  $("accountBtn").onclick = () => {
    renderAccountInfo();
    // 弹窗里的“退出登录”是否可用按打开时的任务状态刷新，避免入口放开了、说明却是旧的。
    syncAccountActions();
    $("accountName").value = account.name || "";
  ...
  // 不能直接赋 openRedeem：onclick 会把 MouseEvent 当作 reason 传入，导致提示条显示 [object MouseEvent]。
  $("creditsBtn").onclick = () => openRedeem();
  $("redeemOpenBtn").onclick = () => openRedeem();
  // 二维码与页脚“联系我”同源：加载成功才露出保存入口，失败可关闭重开重试。
  $("redeemQR").onload = () => {
    $("redeemQRStatus").hidden = true;
    $("saveRedeemQR").hidden = false;
  };
  $("redeemQR").onerror = () => {
    $("redeemQRStatus").textContent = NETWORK_ERROR_MESSAGE;
    $("redeemQRStatus").hidden = false;
    $("saveRedeemQR").hidden = true;
    $("redeemQR").removeAttribute("src");
  };
  ...
  $("emailForm").onsubmit = (e) => {
    ...
    busyButton($("verifyBtn"), async () => {
      ...
      // 账号合并后云端可能多出另一账号的定稿与作品，同步进本机。
      await syncDrafts().catch(() => {});
      await syncWorks().catch(() => 0);
      await syncCommunityShares();
      await renderGallery();
      $("accountDialog").close();
...
async function init() {
  try {
    ...
    renderServiceStatus();
    await restoreProfile();
    // 已有本机图片先展示，不等待云端列表与新图片同步完成。
    await renderGallery();
    // 云端同步其他设备生成的定稿；失败不影响使用本机已有数据。
    await syncDrafts().catch(() => {});
    // 云端同步其他设备生成的作品集；失败不影响使用本机已有数据。
    const syncedWorks = await syncWorks().catch(() => 0);
    // 已分享到社区的作品：作品集里显示「取消分享」，失败不影响本机使用。
    await syncCommunityShares();
    // 没有本地定稿、或只上传过照片时，同步隐藏定稿列表。
    renderCandidates();
    await renderGallery();
    if (syncedWorks > 0) toast("已从云端同步你在其他设备生成的作品。");
    updateControls();
    await resume();
  } catch (e) {
