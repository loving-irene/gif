...
function updateControls() {
  renderEstimates();
  // 任务在服务器后台异步执行：等待中不再锁定生成按钮，可继续调整并提交下一个任务，
...
async function makeMotions() {
  ...
  if (selectedActions.size > availableCredits()) {
    openRedeem(
      `已选 ${selectedActions.size} 个动作，超过剩余 ${Math.max(0, availableCredits())} 次。可减少动作，或扫下方二维码添加微信增加次数。`,
    );
    return;
  }
  if (taskLimitReached()) {
