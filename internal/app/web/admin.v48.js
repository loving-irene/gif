...
function openGalleryPreview(url, alt) {
  const dialog = $("galleryPreviewDialog");
  ...
  galleryPreviewDispose = watchMediaImage(image, url, dialog);
  if (!dialog.open) dialog.showModal();
}
function galleryPreviewTrigger(url, label, alt, className = "") {
  const button = element("button", {
    type: "button",
    className: className || "admin-gallery-main-link",
    title: label,
  });
  button.onclick = () => openGalleryPreview(url, alt);
  return button;
}
function galleryCard(item, kind) {
  ...
  if (item.imageURL) {
    ...
    const trigger = galleryPreviewTrigger(item.imageURL, mainLabel, alt);
