...
function updateControls() {
  ...
  $("motionBtn").disabled =
    !count || !selfie || !catalog?.configured || atLimit;
...
function step(n) {
  ...
  // 有自拍即可选动作；定稿在生成管线中自动完成。
  const ready = Boolean(selfie);
...
async function restoreProfile() {
  ...
  renderCandidates();
  if (selfie) showActions();
  else if (profile.draft) showDraft();
  else step(1);
}
...
function events() {
  ...
  $("step2").onclick = () => (selfie ? showActions() : step(2));
