...
const tasks = new Map();
let taskClock = null;
// 动作批次状态：动作按批次串行执行，但不再阻止同时提交新的定稿任务。
let motionBatch = null;
...
function updateTaskClock() {
  for (const task of tasks.values()) {
    ...
    let text = elapsedEstimateText(task.estimate, elapsed);
    if (task.queueRemaining > 0)
      text += ` · 之后还有${task.queueRemaining}个动作，${estimateText(catalog?.estimates?.motion, task.queueRemaining)}`;
    task.estimateEl.textContent = text;
  }
}
...
function renderServiceStatus() {
  ...
  $("chargeHint").textContent =
    ...
    `当前账号最多可同时执行 ${catalog.userConcurrency || 5} 个任务，等待中也能继续修改造型并提交新的定稿。`;
}
...
function taskLimitReached() {
  const limit = Number(catalog?.userConcurrency) || 5;
  return activeCreditTasks() >= limit;
}
...
async function saveTasks() {
  ...
  if (list.length) await local("pending", "put", { uid: account.id, tasks: list });
...
async function makeDraft() {
  try {
    if (taskLimitReached()) {
      toast(
        `当前账号已有 ${activeCreditTasks()} 个任务在执行，请等待部分任务完成后再提交`,
      );
      return;
    }
...
async function makeMotions() {
  if (motionBatch || !selectedActions.size) return;
  ...
  try {
    ...
    for (let i = 0; i < actions.length; i++) {
      if (motionBatch.paused) break;
      const action = actions[i];
      const task = newTask({
        kind: "motion",
        name: action.name,
        title: `正在制作 ${i + 1}/${actions.length} · ${action.name}`,
        detail:
        ...
        input: {
          ...
          action: action.id,
          selfie: await blobData(source.selfie),
          draft: await blobData(source.draft),
          receipt: source.receipt,
        },
      });
      task.queueRemaining = actions.length - i - 1;
      startTask(task);
      await saveTasks().catch(() => {});
      try {
        const result = await poll(task);
...
function events() {
  ...
  $("stopBtn").onclick = () => {
    if (!motionBatch) return;
    motionBatch.paused = true;
    // 暂停后不再提示后续动作的预计时间。
    for (const task of tasks.values()) task.queueRemaining = 0;
    updateTaskClock();
