...
func TestAdminGalleryListsExistingAssetsAndProtectsBlobs(t *testing.T) {
	...
	if _, err := a.db.Exec("INSERT INTO works(id,user_id,created,name,category,action,gif) VALUES(?,?,?,?,?,?,?)", "gallery-work", user.User.ID, 101, "蓄力攻击", "male", "attack", []byte("GIF89a")); err != nil {
		t.Fatal(err)
	}
