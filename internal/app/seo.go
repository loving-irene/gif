...
var siteFAQs = []faqItem{
	{"如何用自拍生成 GIF 动图？", "上传一张清晰自拍，进入选择动作：先切换男生、女生、小朋友或日常分类，再勾选一个或多个动作。首次生成会自动做出角色定稿，再做成循环 GIF；已有定稿时换动作即可继续生成。每个动作由服务器在后台合成可循环播放的 GIF，完成后可以下载。"},
	{"支持哪些人物分类和表情动作？", "动作按男生、女生、小朋友、日常四个分类展示，切换分类即可看到对应动作。具体动作名称以创作页面当前提供的选项为准，可一次多选并并行制作。"},
	{"照片会保留本人的特点吗？", "角色生成会优先保留脸型、五官关系、发型、肤色与清晰可见的个人特点。AI 生成可能出现差异，可在「我的定稿」里查看与切换；不满意可换自拍后重新生成。"},
	{"上传照片有什么要求？", "支持 JPG、PNG 和 WebP。不超过 5 MB 的照片按原文件上传，不压缩、不缩放或修改格式；仅超过 5 MB 时由浏览器压缩到限制内。建议使用光线均匀、面部清晰、无遮挡且不过度美颜的自拍。"},
	{"生成 GIF 如何计算创作次数？", "生成一次角色定稿、以及生成每个动作，各计一次创作次数。已有定稿后只换动作，则只按所选动作计次。查询进度、本地合成 GIF 和下载不扣次数；失败是否计次以创作页面显示的规则为准。"},
	{"GIF 保存在哪里？换设备能看到吗？", "同一账号可跨设备同步云端定稿和作品。云端定稿保留最近 30 张，按数量淘汰；云端作品最多 30 件，保留最多 3 天。主动分享到社区的图片独立保存，不随作品到期删除，可取消分享，超过 60 条时淘汰最旧分享。本机自拍和作品保存在当前浏览器，清理浏览器数据可能丢失，请及时下载。服务器暂存任务输入以支持后台生成和重启恢复，任务结束后删除输入，残留输入最多保留 3 天。创作时照片会发送给 OpenRouter 图像服务处理，其数据处理以第三方服务约定为准。"},
}
