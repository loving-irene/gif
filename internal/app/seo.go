...
const siteDescription = "拾光 GIF 将自拍制作成专属角色与动态表情包，支持古风铠甲、可爱日常和童趣风格。先确认人物定稿，再选择多个动作，生成循环 GIF 并下载保存。"
...
var siteFAQs = []faqItem{
	{"如何用自拍生成 GIF 动图？", "上传一张清晰自拍，选择人物分类、服装和配色，生成并确认角色定稿，再选择动作。每个动作生成连续画格，由浏览器合成可循环播放的 GIF，完成后可以下载。"},
	{"支持哪些人物造型和表情动作？", "提供男生古代铠甲武将、女生可爱日常、小朋友童趣三类造型。可选择服装与配色，武将可选择武器；动作以创作页面当前提供的选项为准。"},
	...
	{"GIF 保存在哪里？换设备能看到吗？", "自拍、定稿和 GIF 保存在当前设备的浏览器中，本站服务器不持久保存图片。邮箱登录同步账号与次数，不跨设备同步图片，请及时下载。创作时照片会发送给 GeekAI 图像服务处理，其数据处理以第三方服务约定为准。"},
}
