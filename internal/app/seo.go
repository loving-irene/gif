...
var siteFAQs = []faqItem{
	...
	{"GIF 保存在哪里？换设备能看到吗？", "同一账号可跨设备同步云端定稿和作品。云端定稿保留最近 30 张，按数量淘汰；云端作品最多 30 件，保留最多 3 天。主动分享到社区的图片独立保存，不随作品到期删除，可取消分享，超过 60 条时淘汰最旧分享。本机自拍和作品保存在当前浏览器，清理浏览器数据可能丢失，请及时下载。服务器暂存任务输入以支持后台生成和重启恢复，任务结束后删除输入，残留输入最多保留 3 天。创作时照片会发送给 GeekAI 图像服务处理，其数据处理以第三方服务约定为准。"},
}
