...
func TestDebugLogsExplainProviderFailureWithoutSecrets(t *testing.T) {
	...
	cfg, _ := a.settings()
	_, err := a.callProvider(context.WithValue(context.Background(), debugTraceKey{}, "job-test"), cfg, prompt, []string{photo})
...
func TestDebugOffAndNetworkCausePreserved(t *testing.T) {
	...
	cfg, _ := a.settings()
	_, err := a.callProvider(context.Background(), cfg, "prompt", []string{sampleImage(false)})
