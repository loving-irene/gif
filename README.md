...
3. 上传自拍后必选画风：默认、Q版、水墨风格三项，默认画风为极致大头 5:1 比例。画风同时作用于角色定稿与动作图，并写入定稿凭证，改画风须重新生成定稿。再必选男生/女生/小朋友/日常、服装、配色；男生必须选武器。男生古代铠甲（含开枪射击），女生与儿童可爱风，日常为情侣/年轻人聊天贴纸动作。
4. 一次生成定稿返回一张PNG，包含脸部近景和全身造型。用户确认后才允许制作动作。已生成的定稿（含重新生成的，最多保留30张）集中展示在“开始你的小小创作”区顶部，与当前处于第几步无关，第1、2、3步都能看到并点击切换回对应定稿。
5. 动作多选后按账号并发额度并行提交：一次把额度内的动作一起发出去，不再一个一个等前一个做完。服务器同时执行的生成任务数以 `generationSlots` 为上限（当前2个），超出的按创建顺序排队；任意一个动作完成后立刻补提交下一个，直到发完。有动作正在制作时也能继续勾选动作再提交一批，批次之间互不阻塞，只受账号并发上限与剩余次数限制。一个动作生成一张动作序列图：4×4和5×5规格请求上游输出1024×1024方形网格图，10×10规格单独请求2048×2048以增加每个源格的有效像素。切格规格由后台配置：默认 **5×5 共25格、每帧128×128、70ms**；可选 4×4 共16格、每帧256×256、80ms，或 10×10 共100格、最终每帧256×256、20ms（不能整除时按比例取整划分格边界）。动作请求图序为定稿在前、自拍在后。服务器与浏览器本机合成都按同一规格切格，先通过循环五帧局部中位轨迹轻量稳定人物水平中心、脚底与尺寸，再用共享调色板合成无限循环GIF。5×5和10×10提示词额外要求覆盖同一个完整动作、等时间采样、相邻帧小步长变化和人物位置尺寸稳定；规格说明仍会覆盖模板里写死的图片尺寸、格数描述。合成不额外调用AI。动作批次不影响同时提交定稿任务。
...
9. 管理后台 `/who` 支持新账号额度、模型、质量、动作序列图规格（4×4/5×5/10×10）、API密钥、阿里云邮件推送（含反馈接收邮箱）、三种画风提示词、四类服装配色武器、各步骤提示词和动作内容、兑换码、账号停用/增加次数、审计记录、兑换记录（注册赠送、兑换码兑换、后台手动增加三类次数来源，按时间倒序可搜索；升级前数据自动回溯，其中注册赠送为估算值并已标注），以及只读的部署版本检查。数据看板可点击“发送邮件”即时重新汇总并推送完整看板，收件人沿用反馈接收邮箱，留空时回落到发件邮箱。动作名称与过程在同一卡片中编辑，桌面每行3张，平板2张、手机1张；支持新增/删除，每类1—12个动作，点击保存后生效。
...
| 编号 | 名称 | 说明 | 画风效果 |
| --- | --- | --- | --- |
| `default` | 默认 | 极致大头 · 5:1比例 | 头:身体 = 5:1（头约占整高83%），面部对应本人 |
| `chibi` | Q版 | 大头小身 · 更可爱 | 在 5:1 基础上五官更可爱，仍禁止 2/3 头身 |
| `ink` | 水墨风格 | 宣纸墨色 · 写意国风 | 水墨写意 + 极致 Q 版 5:1 比例 |
...
**服务器图片保存**：任务输入（含自拍）写入数据库同目录的 `files` 文件夹，以支持排队与重启恢复；创建失败和任务正常结束后删除对应输入，孤立文件超过3天由清理器兜底删除。生成结果文件保留最多3天；内存结果缓存最多10分钟、合计64MiB。云端定稿每账号最多30张，目前按数量淘汰，没有3天到期清理；云端作品每账号最多30件，并在3天后清理。用户主动分享的社区图片独立保存，不参加3天清理，可由本人取消分享；单张分享图片上限8MiB，单账号超过60条时淘汰最旧分享。
...
**第三方处理**：照片和提示词会发送给 OpenRouter（及其上游画图厂商）以生成图片，第三方处理受其服务约定约束。
...
首次未配置 OpenRouter 时可以访问页面、兑换和管理，但生成功能关闭，不扣次。

## OpenRouter 配置

后台填写 API Key，选择已接入的 OpenRouter 画图模型（默认 `openai/gpt-image-2.5-sunburst`）。接口固定为 `https://openrouter.ai/api/v1`，图片质量可选 low/medium/high/xhigh/max（主要作用于 GPT Image 族）。多模型横评见管理端 `/compare`。

实现依据：[OpenRouter Image Generation](https://openrouter.ai/docs/guides/overview/multimodal/image-generation)、[模型列表](https://openrouter.ai/models)。

- POST `/images/generations`，单图 `image`，多图 `images`，均传入data URL。
- `async=true`，`retries=0`，`n=1`，定稿及4×4/5×5动作使用`size=1024x1024`，10×10动作使用`size=2048x2048`，PNG，透明背景。
...
上线后打开 `/who` 配置 OpenRouter 和阿里云邮件推送。先在华东1（杭州）邮件推送控制台创建发信地址并设置 SMTP 密码，再把完整发信地址和该密码填入后台；旧 SMTP 密码不会自动复用。也可以在首次启动前配置：
...
- 上游固定 OpenRouter，下载主机白名单、解析后校验公网地址并绑定实际连接IP、防止DNS重绑定/SSRF、拒绝重定向。
...
安装Node时，Go测试还会运行真实浏览器GIF编码器，再用Go标准GIF解码器验证16/25/100帧、尺寸、循环、80ms/20ms时序、主体稳定处理和透明处置。没有Node的生产机跳过该编码器交叉验证；不影响应用部署。
